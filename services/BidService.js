...
const BidService = {
    ...
    async acceptBid(bid_id) {
        try {
            ...
            const auction = await Auction.exists({ _id: bid.auctionId, auctionStatus: "closed" });
    ...
    async getAuctions(query) {
        try {
            ...
            return await Auction.find(query).populate([{
                    path: 'bidItems',
                    populate: [{ path: 'buyer' }]
                }])
                .populate('farmer')
    ...
    async createBidTimeline(bid, acceptedPurchase = false, purchasedDetails = false) {
        try {


            let bidTimeline = BidTimeline.findOne({ bidId: bid._id, auctionId: bid.auctionId });

            if (!bidTimeline) {
                bidTimeline = new BidTimeline({ bidId: bid._id, auctionId: bid.auctionId });
            }
    ...
    async updateBidItem(bid_id, data) {
        try {
            return await BidItem.findOneAndUpdate({ _id: bid_id }, {
                $set: data,
            }, { new: true });
        } catch (error) {
