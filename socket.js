...
const BidCroneJob = require('./services/cron/bid.cron');
module.exports = (server) => {
    ...
    socketServer.on('connection', (socket) => {
        ...
        socket.on('bidAccepted', async(buyerId) => {
            ...
            socketServer.to(`buyer:${buyerId}`).emit('bidAcceptedPopup', { bid, autionSettings });
        });
        ...
        socket.on("acceptedPurchase", async(bidId) => {
            ...
            `;
            socketServer.to(`buyer:${bid.buyerId}`).emit("buyerAcceptedPurchase", { message, })
        })
    });
};